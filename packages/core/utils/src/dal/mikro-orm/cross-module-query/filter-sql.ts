...
export type JoinSqlContext = {
  ...
  childrenByAnchor: Map<string, CrossModuleJoinSpec[]>
...
export function joinRequiresFilter(
  ...
  const children = childrenByAnchor.get(joinSpec.alias) ?? []
  return children.some((child) => joinRequiresFilter(child, childrenByAnchor))
}
...
function buildExistsSql(
  ...
  const children = context.childrenByAnchor.get(joinSpec.alias) ?? []
  for (const child of children) {
    if (!joinRequiresFilter(child, context.childrenByAnchor)) {
      continue
    }
