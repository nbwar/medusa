...
export function mikroOrmBaseRepositoryFactory<const T extends object>(
  ...
  class MikroOrmAbstractBaseRepository_ extends MikroOrmBaseRepository<T> {
    ...
    private prepareFindOptions(
      ...
      // Ensure the internal cross-module metadata never leaks into MikroORM's
      // find options (e.g. when an empty array was passed and augmentation was
      // skipped above).
      if (findOptions_.options?.__internal?.crossModuleJoins) {
        delete findOptions_.options.__internal.crossModuleJoins
      }

      if (
        findOptions_.options?.__internal &&
        !Object.keys(findOptions_.options.__internal).length
      ) {
